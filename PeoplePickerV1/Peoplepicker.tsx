...
export class PeoplePickerTypes extends BaseComponent<any, IPeoplePickerState> {
  ...
  private _searchUsers(filterText: string): IPersonaProps[] | Promise<IPersonaProps[]> {
    return new Promise(async (resolve: any, reject: any) => {
      ...
      try {
        let tempPeople: any = [];
        if (this.props.context.parameters.entityName.raw! === "systemuser") {
          // systemuser --> FullName = FullName and Primary Email = InternalEMailAddress
          ...
          await Promise.all(tempPeople.entities.map((entity: any) => {
            ...
            }); //change fieldname if values are different
          }));
        }
          ...
          await Promise.all(tempPeople.entities.map((entity: any) => {
            ...
            if (this.props.context.parameters.loadEnabledAccounts.raw! === 'true') {
              // check accountenabled or not
              if (entity.accountenabled === true) {
                People.push({
